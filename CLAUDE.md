...
**Modular** — one responsibility per file. Split when a file handles more than one concern or grows past ~150 lines. Each module should be replaceable without touching others. Boundaries are load-bearing: `scene.js`, `rake.js`, `input.js` are the model — not a catch-all `utils.js`.
