...
async function buildTSLScene() {
  ...
  let lastDirX = 1, lastDirZ = 0;
  createPointerInput(camera, renderer.domElement, {
    onMove(x, z, dx, dz) {
      const len = Math.sqrt(dx * dx + dz * dz);
      if (len > 0.001) { lastDirX = dx / len; lastDirZ = dz / len; }
      rakeGroup.position.set(x, 0, z);
      rakeGroup.rotation.y = Math.atan2(lastDirX, lastDirZ);
      uRakePos.value.set(x, z);
      uRakeDir.value.set(lastDirX, lastDirZ);
    },
    onUp() { uRakeSpeed.value = 0; },
  });
...
function buildGLSLFallback() {
  ...
  createPointerInput(camera, renderer.domElement, {
    onMove(x, z, dx, dz) {
      ...
      rakeGroup.position.set(x, 0, z);
      rakeGroup.rotation.y = Math.atan2(lastDirX, lastDirZ);
      velUniforms.uRakePos.value.set(x, z);
      velUniforms.uRakeDir.value.set(lastDirX, lastDirZ);
    },
    onUp() { velUniforms.uRakeSpeed.value = 0; },
  });
